...
public class BenchmarkRunner
{
	...
	public static void main(String[] args)
	{
		...
		// Binary Formats
		// start with baseline, Java defaults (to reduce first-run benefit for
		// other impls)
		...
		TestGroup<?> group;
		{
			...
			if (loader == null) {
				System.out.println("Data file \"" + dataFileName + "\" can't be loaded.");
				System.out.println("No deserializer registered for data type \"" + dataType
						+ "\" and file extension \"." + extension + "\"");
				System.exit(1);
				return;
			}
	...
	private static final TestCase Create = new TestCase() {
		public <J> double run(Transformer<J, Object> transformer, Serializer<Object> serializer, J value, int iterations)
				throws Exception
		{
	...
	{
		public abstract <J> double run(Transformer<J, Object> transformer, Serializer<Object> serializer, J value,
	...
	enum measurements {
		timeCreate("create"), timeSerializeDifferentObjects("ser"), timeSerializeSameObject("ser+same"), timeDeserializeNoFieldAccess(
				"deser"), timeDeserializeAndCheck("deser+deep"), timeDeserializeAndCheckShallow("deser+shal"), totalTime(
				"total"), length("size"), lengthDeflate("size+dfl"), ;
	...
	private static <J> EnumMap<measurements, Map<String, Double>> start(PrintWriter errors, int iterations, int trials,
			long warmupTime, boolean prewarm, Iterable<TestGroup.Entry<J, Object>> groups, J value) throws Exception
	{
		...
		for (TestGroup.Entry<J, Object> entry : groups) {
			...
			try {
				...
				/*
				 * Should only warm things for the serializer that we test next: HotSpot JIT
				 * will otherwise spent most of its time optimizing slower ones... Use
				 * -XX:CompileThreshold=1 to hint the JIT to start immediately
				 * 
				 * Actually: 1 is often not a good value -- threshold is the number of samples
				 * needed to trigger inlining, and there's no point in inlining everything.
				 * Default value is in thousands, so lowering it to, say, 1000 is usually
				 * better.
				 */
				warmCreation(runner, warmupTime);

				doGc();
				double timeCreate = runner.runTakeMin(trials, Create, iterations * 100); // do
				// more
				// iteration
				// for
				// object
				...
				doGc();
				double timeDeserializeAndCheckShallow = runner.runTakeMin(trials, DeserializeAndCheckShallow,
						iterations);
				...
				System.out.printf("%-32s %6.0f %7.0f %7.0f %7.0f %7.0f %7.0f %7.0f %6d %5d\n", name, timeCreate,
						...
						timeDeserializeAndCheckShallow, timeDeserializeAndCheck, totalTime, array.length,
						compressDeflate.length);

				addValue(values, name, timeCreate, timeSerializeDifferentObjects, timeSerializeSameObject,
						timeDeserializeNoFieldAccess, timeDeserializeAndCheckShallow, timeDeserializeAndCheck,
						totalTime, array.length, compressDeflate.length);
			}
			catch (Exception ex) {
	 ...
	 */
	private static <J> void checkCorrectness(PrintWriter errors, Transformer<J, Object> transformer,
			Serializer<Object> serializer, J value) throws Exception
	{
		Object specialInput;
	...
	private static void printImage(Map<String, Double> map, measurements m)
	{
		...
		// Reduce bar thickness and spacing until we can fit in the maximum
		// height.
		while (true) {
		...
		System.out
				.println("<img src='http://chart.apis.google.com/chart?chtt="
						+ urlEncode(m.displayName)
						+ "&chf=c||lg||0||FFFFFF||1||76A4FB||0|bg||s||EFEFEF&chs="
						+ width
						+ "x"
						+ height
						+ "&chd=t:"
	...
	private static void addValue(EnumMap<measurements, Map<String, Double>> values, String name, double timeCreate,
			double timeSerializeDifferentObjects, double timeSerializeSameObject, double timeDeserializeNoFieldAccess,
			double timeDeserializeAndCheckShallow, double timeDeserializeAndCheck, double totalTime, double length,
			double lengthDeflate)
	{
